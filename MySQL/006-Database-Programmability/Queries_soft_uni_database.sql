...
CREATE FUNCTION `ufn_calculate_future_value`(`sum` DECIMAL (20,4), `yearly_interest_rate` DECIMAL (20,4),
                                             `number_of_years` INT)
...
`result` DECIMAL(20,4);
    SET
result := `sum` * POW(1 + `yearly_interest_rate`, `number_of_years`);
RETURN `result`;
END;
